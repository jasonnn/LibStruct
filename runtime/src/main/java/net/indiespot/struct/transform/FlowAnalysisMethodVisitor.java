...
@SuppressWarnings({"ForLoopReplaceableByForEach", "ObjectEquality"})
public class FlowAnalysisMethodVisitor extends MethodVisitor {
    ...
    public void visitTypeInsn(int opcode, String type) {
        ...
        switch (opcode) {
            case NEW:
                if (type.equals(StructEnv.Descs.PLAIN_STRUCT_FLAG))
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
                ...
                if (stack.peek(ii) == VarType.STRUCT) {
                    if (descToType(arr[i]) == A_REFERENCE) {
                        if (owner.startsWith("java/util/")) {
                            if (owner.endsWith("List") || //
                                ...
                                throw new UnsupportedOperationException(owner + '.' + name + desc +//
                                        ...
                                        "\nYou can generate struct-backed collection-classes using: StructGenericsSourcecodeGenerator");
                            }
                        }
                        ...
                        if (i == arr.length - 1 || i == arr.length - 3) {
                            ...
                            // ...,b,a,struct
                            this.visitMethodInsn(INVOKESTATIC, StructMemory.class.getName().replace('.', '/'), "toString", '(' + StructEnv.Descs.WRAPPED_STRUCT_FLAG + ')' + arr[i], itf);
                            // ...,b,a,string
        ...
        if (desc.contains(StructEnv.Descs.WRAPPED_STRUCT_FLAG)) {
            desc = desc.replace(StructEnv.Descs.WRAPPED_STRUCT_FLAG, "I");
            if (StructEnv.PRINT_LOG)
    ...
    @SuppressWarnings("ObjectEquality") //TODO ?
    @Override
    ...
    public static String[] splitDescArray(String desc) {
    ...
    private static String getFirstType(String desc) {
    ...
    private static int descToType(String desc) {
        ...
        if (desc.equals("D"))
            return A_LONG_OR_DOUBLE;
        if (desc.equals(StructEnv.Descs.WRAPPED_STRUCT_FLAG))
            return A_STRUCT;
        if (desc.equals('[' + StructEnv.Descs.WRAPPED_STRUCT_FLAG))
